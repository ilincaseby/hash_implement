...
ht_create(unsigned int hmax, unsigned int (*hash_function)(void*),
		int (*compare_function)(void*, void*))
{
	hashtable_t *dict = malloc(sizeof(*dict));
	...
	dict->buckets = malloc(hmax * sizeof(*(dict->buckets)));
	if (!dict->buckets) {
		fprintf(stderr, "Malloc failed\n");
		return NULL;
	}
...
int verify_ver(hashtable_t *ht, void *key)
{
	if (!ht)
		return 0;
	if (!key)
	...
	void *value, unsigned int value_size)
{
	...
	if (have_been_there) {
		...
		memcpy(value_to_be_changed, value, value_size);
		return ;
	}
